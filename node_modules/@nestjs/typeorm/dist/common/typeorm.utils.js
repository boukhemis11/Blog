...
function handleRetry(retryAttempts = 9, retryDelay = 3000, connectionName = typeorm_constants_1.DEFAULT_CONNECTION_NAME) {
    return (source) => source.pipe(operators_1.retryWhen(e => e.pipe(operators_1.scan((errorCount, error) => {
        const connectionInfo = connectionName === typeorm_constants_1.DEFAULT_CONNECTION_NAME
            ? ''
            : ` (${connectionName})`;
        logger.error(`Unable to connect to the database${connectionInfo}. Retrying (${errorCount +
            1})...`, error.stack);
